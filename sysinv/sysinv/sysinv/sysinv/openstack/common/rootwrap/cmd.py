...
"""Root wrapper for OpenStack services
   ...
   To use this with sysinv, you should set the following in
   sysinv.conf:
   rootwrap_config=/etc/sysinv/rootwrap.conf

   You also need to let the sysinv user run sysinv-rootwrap
   as root in sudoers:
   sysinv ALL = (root) NOPASSWD: /usr/bin/sysinv-rootwrap
                                   /etc/sysinv/rootwrap.conf *
...
def main():
    ...
    possible_topdir = os.path.normpath(os.path.join(os.path.abspath(execname),
                                                    os.pardir, os.pardir))
    if os.path.exists(os.path.join(possible_topdir, "sysinv", "__init__.py")):
        sys.path.insert(0, possible_topdir)

    from sysinv.openstack.common.rootwrap import wrapper

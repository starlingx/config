...
class TestHost(base.FunctionalTest, dbbase.BaseHostTestCase):
    ...
    def _change_oam_to_ipv6(self):
        network = self._find_network_by_type(constants.NETWORK_TYPE_OAM)
        self.dbapi.network_destroy(network.id)
        self._create_test_addresses([constants.CONTROLLER_GATEWAY,
                                     constants.CONTROLLER_HOSTNAME,
                                     constants.CONTROLLER_0_HOSTNAME],
                                    [dbbase.OAM_SUBNET_IPV6], constants.NETWORK_TYPE_OAM)
        self._create_test_network('oam', constants.NETWORK_TYPE_OAM, [dbbase.OAM_SUBNET_IPV6],
                                  link_addresses=True)
...
class TestPostFirstController(TestHost):
    ...
    def _test_create_host_invalid_bm_ip_wrong_version(self, invalidip):
        # Test creation of host with bm_ip with different version from OAM primary IP
        ndict = dbutils.post_get_test_ihost(hostname='controller-0',
                                            bm_ip=invalidip,
                                            bm_type=constants.HOST_BM_TYPE_IPMI,
                                            bm_username="root")
    ...
    def test_create_host_invalid_bm_ip_wrong_version_ipv4(self):
        self._test_create_host_invalid_bm_ip_wrong_version("fd01::77")

    def test_create_host_invalid_bm_ip_wrong_version_ipv6(self):
        self._change_oam_to_ipv6()
        self._test_create_host_invalid_bm_ip_wrong_version("10.10.10.100")
...
class TestSimplexPostKubeUpgrades(TestHost):
    system_mode = constants.SYSTEM_MODE_SIMPLEX

    def setUp(self):
        super(TestSimplexPostKubeUpgrades, self).setUp()
...
class TestPatch(TestHost):
    ...
    def _test_update_host_invalid_bm_ip_version(self, bm_ip):
        # Test updating a host with bm_ip with different version from OAM primary IP
    ...
    def test_update_host_invalid_bm_ip_version_ipv4(self):
        self._test_update_host_invalid_bm_ip_version('fd01::77')

    def test_update_host_invalid_bm_ip_version_ipv6(self):
        self._change_oam_to_ipv6()
        self._test_update_host_invalid_bm_ip_version('10.10.10.100')

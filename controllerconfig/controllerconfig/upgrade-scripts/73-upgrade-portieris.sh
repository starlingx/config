...
if [[ $EXISTING_APP_STATUS != "applied" && $EXISTING_APP_STATUS != "uploaded" ]]; then
    log "${UPGRADE_APP_NAME}, version ${EXISTING_APP_VERSION}, not in upgradeable state: ${EXISTING_APP_STATUS}, skipping"
    exit 0
fi

log "portieris, version $EXISTING_APP_VERSION, is currently in the state: $EXISTING_APP_STATUS"
...
if [ "x${UPGRADE_APP_VERSION}" != "x${EXISTING_APP_VERSION}" ]; then
    ...
    if [[ $EXISTING_APP_STATUS == "applied" ]]; then
        ...
        log "Exporting portieris-certs overrides"
        ...
        if [ ! -f "$CONFIG_PERMDIR/.portieris_upgrade_dump" ]; then
            ...
            if [ "${EXISTING_PORTIERIS_RESOURCES}" == 'No resources found' ]; then
                log "no existing portieris resources detected."
                ...
                if [ $? != 0 ]; then
                    log "Failed to dump existing portieris resources. Exiting for manual intervention..."
                    exit 1
                fi
            fi
        ...
        if [ ! -f "$CONFIG_PERMDIR/.portieris_upgrade_convert" ]  && \
           [ ! -f "$CONFIG_PERMDIR/.portieris_upgrade_no_existing_resources" ]; then
            log "converting portieris resources backup"
            ...
            if [ $? != 0 ]; then
                log "Failed to convert portieris resources. Exiting for manual intervention..."
                exit 1
            fi
            ...
            if [ $? != 0 ]; then
                log "Failed to delete creationTimestamp in portieris export. Exiting for manual intervention..."
                exit 1
            fi

            sed -i '/resourceVersion:/d' $CONFIG_PERMDIR/portieris-converted.yaml
            if [ $? != 0 ]; then
                log "Failed to delete resourceVersion in portieris export. Exiting for manual intervention..."
                exit 1
            fi

            sed -i '/selfLink:/d' $CONFIG_PERMDIR/portieris-converted.yaml
            if [ $? != 0 ]; then
                log "Failed to delete selfLink in portieris export. Exiting for manual intervention..."
                exit 1
            fi

            sed -i '/uid:/d' $CONFIG_PERMDIR/portieris-converted.yaml
            if [ $? != 0 ]; then
                log "Failed to delete uid in portieris export. Exiting for manual intervention..."
                exit 1
            fi
        ...
        if [ $tries == $REMOVE_RESULT_ATTEMPTS ]; then
            log "${EXISTING_APP_NAME}, version ${EXISTING_APP_VERSION}, was not removed in the allocated time. Exiting for manual intervention..."
            exit 1
        fi
        ...
        if [ $tries == $DELETE_RESULT_ATTEMPTS ]; then
            log "${EXISTING_APP_NAME}, version ${EXISTING_APP_VERSION}, was not deleted in the allocated time. Exiting for manual intervention..."
            exit 1
        fi


        # upload new portieris
        log "Uploading ${UPGRADE_APP_NAME}, version ${UPGRADE_APP_VERSION} from $UPGRADE_PORTIERIS_TARBALL"
        ...
        if [ $tries == $UPLOAD_RESULT_ATTEMPTS ]; then
            log "${UPGRADE_APP_NAME}, version ${UPGRADE_APP_VERSION}, was not uploaded in the allocated time. Exiting for manual intervention..."
            exit 1
        fi
        ...
        if [ -f "$CONFIG_PERMDIR/portieris-certs-overrides.yaml" ]; then
            ...
            if [ $? != 0 ]; then
                log "Failed to apply portieris overrides. Exiting for manual intervention..."
                exit 1
            fi
        ...
        log "adding portieris image to local registry"
        IMAGE="portieris/portieris:v0.13.1"
        system registry-image-tags icr.io/portieris/portieris| grep 'v0\.13\.1'
        if [ $? -eq 0 ]; then
            log "image ${IMAGE} already exists, nothing needs to be done."
        else
            ...
            if [ $? -eq 0 ]; then
                ...
                log "logging in to docker registry using credentials stored in barbican"
            ...
            system registry-image-tags icr.io/portieris/portieris | grep 'v0\.13\.1'
            if [ $? -ne 0 ]; then
                log "$IMAGE not tagged correctly, exiting for manual intervention"
                exit 1
            else
                log "done."
            fi
        ...
        for tries in $(seq 1 $APPLY_RESULT_ATTEMPTS); do
            ...
            if [ "${UPGRADE_APP_STATUS}" == 'applied' ]; then
                log "${UPGRADE_APP_NAME} has been applied."
                break
            fi
        ...
        if [ $tries == $APPLY_RESULT_ATTEMPTS ]; then
            log "${UPGRADE_APP_NAME}, version ${UPGRADE_APP_VERSION}, was not applied in the allocated time. Exiting for manual intervention..."
            exit 1
        fi
        ...
        if [ ! -f "$CONFIG_PERMDIR/.portieris_upgrade_no_existing_resources" ]; then
            ...
            if [ $? != 0 ]; then
                log "Failed to apply portieris resources on the fluxcd version of portieris. Exiting for manual intervention..."
                exit 1
            fi
        fi
